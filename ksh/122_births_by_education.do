...
collapse (sum) births, by(cohort age mother_educ)
drop if cohort == .


reshape wide births_, i(age cohort) j(mother_educ)
...
forval i = 1970(1)1984 {
	#d ;
		line f_cumulative_1_`i' f_cumulative_2_`i' f_cumulative_3_`i' f_cumulative_4_`i' f_cumulative_5_`i'  age,
			lcolor($color1 $color2 $color3 $color4 $color5)
			lpattern(solid _ - solid _)
			graphregion(color(white))
			legend(order( 1 "less than 8 grades" 2 "8 grades" 3 "vocational" 4 "high school diploma" 5"tertiary"))
			xtitle("Age")
