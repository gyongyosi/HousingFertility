...
* save 
save "${temp}/lbForMatch_2plus_D", replace
...
joinby `matchingVars' using "${temp}/lbForMatch_2plus_D", unmatched(master) update
tab _merge
...
keep szemazon momID
sort momID szemazon
save "${temp}/matches_2plus_D", replace
...
merge 1:1 momID using "${temp}/matches_2plus_D"
keep if _merge==1
drop _merge
* check duplicates
duplicates tag `matchingVars', gen(tag)
...
rename loc_mother loc_mother_lb
rename edu_mother edu_mother_lb
save "${temp}/lbForMatch_2plus_M", replace
...
merge 1:1 szemazon using "${temp}/matches_2plus_D"
keep if _merge==1
drop _merge
* check duplicates
duplicates tag `matchingVars', gen(tag)
...
* old code: merge 1:1 `matchingVars' using "${temp}/lb_panel_for_matching_2D", update
* for now, filling in only missing which keeps census as master
joinby `matchingVars' using "${temp}/lbForMatch_2plus_M", unmatched(master) update
tab _merge
...
keep szemazon momID
sort momID szemazon
save "${temp}/matches_2plus_M", replace

* combine matches from first and second passes  ********************************

use "${temp}/matches_2plus_D", clear
gen case = 1
append using "${temp}/matches_2plus_M"
...
rename loc_mother loc_mother_lb
rename edu_mother edu_mother_lb
save "${temp}/lbForMatch_1_D", replace
...
* old code: merge 1:1 `matchingVars' using "${temp}/lb_panel_for_matching_2D", update
* for now, filling in only missing which keeps census as master
joinby `matchingVars' using "${temp}/lbForMatch_1_D", unmatched(master) update
tab _merge
...
keep if _merge==1
drop _merge
merge 1:1 momID using "${temp}/matches_1_D"
keep if _merge==1
drop _merge
* check for duplicates
duplicates tag `matchingVars', gen(tag)
...
rename loc_mother loc_mother_lb
rename edu_mother edu_mother_lb
save "${temp}/lbForMatch_1_M", replace
...
keep if tm_child_1!=. & tm_child_2==.
* remove szemazon's that matched to LB in first pass w/ exact dates
merge 1:1 szemazon using "${temp}/matches_1_D"
keep if _merge==1
drop _merge
* check for duplicates
duplicates tag `matchingVars', gen(tag)
...
* old code: merge 1:1 `matchingVars' using "${temp}/lb_panel_for_matching_2D", update
* for now, filling in only missing which keeps census as master
joinby `matchingVars' using "${temp}/lbForMatch_1_M", unmatched(master) update
tab _merge
...
keep szemazon momID
sort momID szemazon
save "${temp}/matches_1_M", replace

* combine matches from first and second passes  ********************************
use "${temp}/matches_1_D", clear
gen case = 1
append using "${temp}/matches_1_M"
replace case = 2 if case==.
* check for duplicates: should be none, bc dropping LB and census based on 1st pass
duplicates tag momID, gen(tag)
tab tag
...
drop _merge
* keep live birth entries that did not match before
merge 1:1 momID using "${temp}/matches_1"
keep if _merge==1
drop _merge
* check for duplicates
duplicates tag `matchingVars', gen(tag)
...
* save 
save "${temp}/lbForMatch_0", replace
...
* old code: merge 1:1 `matchingVars' using "${temp}/lb_panel_for_matching_2D", update
* for now, filling in only missing which keeps census as master
joinby `matchingVars' using "${temp}/lbForMatch_0", unmatched(master) update
tab _merge
...
merge 1:1 momID using "${temp}/matches_all"
keep if _merge==3
...
keep if ty_baby > 2022

********************************************************************************
* evaluation via regression
****************************************************************************************
* load all mom-level matches
use "${temp}/matches_all", replace
* merge with my mother-level intra-LB merge to get all the kids info
merge m:1 momID using "${temp}/lb_all"
keep if _merge==3
keep momID id_baby_*
* turn into baby level dataset
reshape long id_baby_, i(momID) j(num)
rename id_baby_ id_baby
drop if id_baby==.
drop num

* merge with baby-level LB data
merge 1:1 id_baby using "${temp}/LB_for_matching_001"
gen match = 0 
replace match = 1 if _merge==3
drop _merge
dtable match, by(y_baby) export("${output}/matchBreakdown.xlsx", replace)
save "${temp}/whichLBs", replace

* reg
use "${temp}/tstar_important", clear
keep if ty==2018
save "${temp}/tstar_important_2018regmerge", replace

use "${temp}/whichLBs", clear
merge m:1 ksh4_bpker using "${temp}/tstar_important_2018regmerge"
gen csok_mother = 0
...
eststo q_3: reg match i.y_baby i.nocsal i.y_father i.y_mother i.edu_mother i.edu_father i.elve1 i.nem i.csok_mother if CSOK_10000!=.
* post-census in treatment and control settlements
eststo q_4: reg match i.y_baby i.nocsal i.y_father i.y_mother i.edu_mother i.edu_father i.elve1 i.nem i.csok_mother if tm_baby >= ym(2022,10) & CSOK_10000!=.

* save 
esttab using "${output}/checkingLBmatchBalance_update.rtf", replace 

egen testing = group(y_baby csok_mother)
keep if CSOK_10000!=.
dtable match, by(testing) export("${output}/matchBreakdownByStatusTOT.xlsx", replace)
