...
use "${csok}/village_csok_law", clear
* tim add 7/16/2026
rename ksh4 ksh4_bpker
rename v2019 village_csok
...
local VARS "fete300 subsidy_1_2016_2023 subsidy_2_2016_2023 subsidy_4_2016_2023 subsidy_3_2019_2023 number_of_contracts_2016_2023 families_csok_2016_2023 existing_kids_2016_2023 planned_kids_2016_2023 tamftcsok1623összesensum201 tamft1623összesensum201620"
...
gen shareWomen_15_49 = women_15_49_TSTAR / de16
gen pop_15_64 =  (de30 + de31 + de32 + de33 + de34 + de35 + de36 + de37) + (de40 + de41 + de42 + de43 + de44 + de45 + de46 + de47)


keep  ksh4 ty de01 de02 de03 de11 de12 jaras175 mkod2018 rkod2018 women_15_49_TSTAR pop_15_64 shareWomen_15_49
...
foreach BW in 25000 20000 15000 10000 5000 4000 3000 2000 {
...
ren tazon ksh4
ren ev ty
foreach X of varlist la* {
	replace `X' = 0 if `X' == .
}
gen numHomes = la01
keep ksh4 ty numHomes
tempfile la1
save `la1'
...
ren tazon ksh4
ren ev ty
foreach X of varlist ok* {
	replace `X' = 0 if `X' == .
}
gen kinderSpots = ok10
keep ksh4 ty kinderSpots
tempfile ok
save `ok'
...
keep ksh4 ty la01 la02 la68 la70 la71 la72 la73
tempfile la2
save `la2'
...
merge m:1 ksh4 ty using `la1', nogen keep(1 3)
merge m:1 ksh4 ty using `la2', nogen keep(1 3)
merge m:1 ksh4 ty using `ok', nogen keep(1 3)
...
* the village_csok file was just a list, so let the ineligible settlements take 0 not .
replace CSOK_0000 = 0 if CSOK_0000 == .
* TIM addition 07/02/2026: 0000 variables that capture actual eligibility, not ITT-style population
rename CSOK_0000 CSOK_0000_all
* rebuild 2018 pop var
gen tmp_de01 = de01 if ty == 2018
egen de01_2018 = mean(tmp_de01), by(ksh4)
drop tmp_de01
* build "0000" CSOK variables that use TOT actual eligibility from village_csok file NOT ITT population
foreach BW in 25000 20000 15000 10000 5000 4000 3000 2000 {

	local lower = 5000 - `BW'
	local upper = 5000 + `BW'
	gen CSOK_0000_`BW' = .
	replace CSOK_0000_`BW' = 1 if (CSOK_0000_all==1 & de01_2018!=. & inrange(de01_2018,`lower',`upper')) 
	replace CSOK_0000_`BW' = 0 if (CSOK_0000_all==0 & de01_2018!=. & inrange(de01_2018,`lower',`upper')) 
}
* drop this var I created bc we re-do it with different version (keep Budapest separate) in a few lines
drop de01_2018

* these are the T-STAR controls I end up using in regressions
gen U = 100*(mn01 / de01)
gen income = (tx02 - tx03) / tx01
gen ln_income = ln(income)
gen emp_SH = emp_all / pop_15_64
gen hsShare = SH_t_3 + SH_t_4
* 2018 snapshots
foreach X in de01 U ln_income hsShare {
	foreach Y in 2018 2012 {
		gen tmp_`X' = `X' if ty == `Y'
		egen `X'_`Y' = mean(tmp_`X'), by(ksh4_bpker)
		drop tmp_`X'
	}
}
** unemployment: binned verions
xtile U_2018_bin5 = U_2018, nq(5)
xtile U_2018_bin4 = U_2018, nq(4)
xtile U_2018_bin3 = U_2018, nq(3)
xtile U_2018_bin2 = U_2018, nq(2)
** log income: binned versions
xtile ln_income_2018_bin5 = ln_income_2018, nq(5)
xtile ln_income_2018_bin4 = ln_income_2018, nq(4)
xtile ln_income_2018_bin3 = ln_income_2018, nq(3)
xtile ln_income_2018_bin2 = ln_income_2018, nq(2)
** log income: binned versions
xtile hsShare_2018_bin5 = hsShare_2018, nq(5)
xtile hsShare_2018_bin4 = hsShare_2018, nq(4)
xtile hsShare_2018_bin3 = hsShare_2018, nq(3)
xtile hsShare_2018_bin2 = hsShare_2018, nq(2)


* order and save
order ksh4 ty jaras mkod rkod CSOK_all ///
	CSOK_25000 CSOK_20000 CSOK_15000 CSOK_10000 ///
	CSOK_5000 CSOK_4000 CSOK_3000 CSOK_2000 ///
	CSOK_0000_all ///
	CSOK_0000_25000 CSOK_0000_20000 CSOK_0000_15000 CSOK_0000_10000 ///
	CSOK_0000_5000 CSOK_0000_4000 CSOK_0000_3000 CSOK_0000_2000
...
xtset ksh4_bpker ty
foreach INCL in  "women1549" {
	foreach BW in "0000" 15000 {
		foreach DIRECTION in "in" "out" {
			foreach ELIG in 0 1 {
				gen q_`INCL'_mig_`BW'_`DIRECTION'_`ELIG' = N_`INCL'_migrate_`BW'_`DIRECTION'_`ELIG' / l.women_15_49
			}
		}
	}
}
